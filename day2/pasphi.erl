...
go(FileName) ->
  ...
  io:format("~p~n", [Lines]),
  Correct = lists:filter(fun({[Lower, Upper], [Letter], Password}) -> is_valid(Lower, Upper, Letter, Password) end, Lines),
  io:format("Correct1 ~p~n", [length(Correct)]),
  Correct2 = lists:filter(fun({[Lower, Upper], [Letter], Password}) -> is_valid2(Lower, Upper, Letter, Password) end, Lines),
  io:format("Correct2 ~p~n", [length(Correct2)]).
